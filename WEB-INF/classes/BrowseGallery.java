...
public class BrowseGallery extends HttpServlet implements SingleThreadModel {
	public void doGet(HttpServletRequest request, HttpServletResponse res)
		...
		try {
			 ...
			 * to see depending on the permissions they chose
			 */
			String query = "";
			...
			if (userName.equals("admin")) {
				...
				query = "Select photo_id from images where permitted='2' and owner_name='"
						+ userName + "'";
			} else if (group_id.equals("top")){
				query = "Select i.photo_id from images i, unique_views uv "+
				"where i.photo_id = uv.photo_id "+
				"group by i.photo_id "+
				"order by count(uv.visitor_id) desc "+
				"Where rownum <=5";
			}
			else {
				query = "Select photo_id from images where permitted='"
						+ group_id + "'";
			}
			...
			while (rset.next()) {
				...
				out.println("<img src=\"/proj1/GetOnePic?thumb" + p_id
						+ "\"></a>");
			}
			stmt.close();
			conn.close();
